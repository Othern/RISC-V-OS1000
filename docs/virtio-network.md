...
9. IPv4 header 驗證、checksum、統計與發包 helper 已拆到 `kernel/ipv4.c`。
10. 再往上做 ICMP ping、UDP 或 TCP。
...
- QEMU user-mode network 不等於 guest 自動有 TCP/IP stack；virtio-net driver 只處理 L2 Ethernet frame。目前已實作基礎 ARP 與初步 IPv4 parser，上層 ICMP/UDP/TCP 仍要自己實作。
